  ...
  const render=()=>{
    ...
    renderRows.forEach(({row,sourceRowIndex,isHeader,isTotal},rowIndex)=>visible.forEach((ci,columnIndex)=>{
      ...
      if(!isHeader&&!isTotal&&(ci===0||ci===1)){
        ...
        if(sku){const trigger=document.createElement('button');trigger.type='button';trigger.className='accounting-chat-trigger';trigger.dataset.chatSku=sku;const title=PRODUCT_NAMES[sourceRowIndex]||sku;trigger.setAttribute('aria-label','Chat about '+title);trigger.title='Chat about '+title;while(el.firstChild)trigger.append(el.firstChild);trigger.addEventListener('click',()=>document.dispatchEvent(new CustomEvent('dolce:product-chat',{detail:{sku,title,opener:trigger}})));el.append(trigger);}
      }
      grid.append(el)
    }));
